...
/**
 * 去重修复（集成 dsh-fix / dev_fix_patch 能力）：同 entry id 重复条目只保留
 * 最后一条，删除前面出现的（连同其上方隔离注释）。duplicate loader entry id
 * 会让 DSH 启动崩溃。写前自动备份。
 * @returns {{ok: boolean, removed: number, file?: string, error?: string}}
 */
export function fixDuplicatePatchIds(profileDir) {
  if (!profileDir) return { ok: false, error: '无法定位 profile 目录' }
  const { file, entries } = readPatchEntries(profileDir)
  if (!file) return { ok: false, error: 'profile patch 文件不存在' }
  const seen = new Set()
  ...
  for (const e of entries) {
    if (seen.has(e.id)) toRemove.push(e)
    else seen.add(e.id)
  }
  if (toRemove.length === 0) return { ok: true, removed: 0, file }
  const lines = readFileSync(file, 'utf8').split('\n')
  backupPatchFile(profileDir)
  // 删除块：entry 行 + 属性行，并向上吸收紧邻的隔离注释；从后往前删避免行号漂移。
  const ranges = toRemove
    .map((e) => {
      let start = e.lineNo - 1
      while (start > 0 && /^\s*#/.test(lines[start - 1])) start--
      return { start, end: (e.lineNo - 1) + e.lines.length }
    })
    .sort((a, b) => b.start - a.start)
  for (const r of ranges) {
    let end = r.end
    while (end < lines.length && lines[end].trim() === '') end++ // 吸收块后空行
    lines.splice(r.start, end - r.start)
  }
  writeFileSync(file, lines.join('\n').replace(/\n{3,}/g, '\n\n').trimEnd() + '\n')
  return { ok: true, removed: toRemove.length, file }
}
