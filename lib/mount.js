/**
 * dsh-fail-soft — mount 兜底模块（被 DSH 内核动态 import，不走 cordis 插件树）。
 *
...
export const QUARANTINE_MARKER = 'dsh-fail-soft'
...
const PLUGIN_TAG = '[fail-soft][plugin dsh-fail-soft]'
...
export function quarantineEntries(profileDir, failures) {
  ...
  try {
    ...
    diag(`dsh-fail-soft: cannot read quarantine target ${patchFile}: ${String(error)}`)
    return []
  }
  ...
  try {
    ...
    diag(`dsh-fail-soft: cannot quarantine into ${patchFile}: ${String(error)}`)
    return []
  }
  return written
}
