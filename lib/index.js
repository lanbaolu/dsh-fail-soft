...
/** 持久化开关文件路径（内核补丁 isFailSoft 同样读取它）。 */
function failSoftSwitchPath() {
  return join(homedir(), '.dsh', 'fail-soft.json')
}

/** 读取持久化开关：文件声明 `{ "enabled": true }` 即视为开启。 */
function readFailSoftSwitch() {
  try {
    const data = JSON.parse(readFileSync(failSoftSwitchPath(), 'utf8'))
...
/** 写入持久化开关（供 App/终端用户无需设置环境变量即可启用 fail-soft）。 */
function writeFailSoftSwitch(enabled) {
  const file = failSoftSwitchPath()
  writeFileSync(file, JSON.stringify({ enabled: !!enabled }, null, 2) + '\n', 'utf8')
  return { ok: true, enabled: !!enabled, file }
}
