...
export function lineDiffInsertions(origLines, officialLines) {
  // 用计数处理重复行：orig 中每个行可被 official 消耗一次，多余的才算新增。
  const origCount = new Map()
  for (const line of origLines) origCount.set(line, (origCount.get(line) ?? 0) + 1)
  const available = new Map(origCount)
  const isOrigLine = (line) => (available.get(line) ?? 0) > 0
  const consume = (line) => {
    const n = (available.get(line) ?? 0) - 1
    if (n <= 0) available.delete(line)
    else available.set(line, n)
  }
  const insertions = []
  let i = 0
  while (i < officialLines.length) {
    const line = officialLines[i]
    if (isOrigLine(line)) {
      consume(line)
      i++
      continue
    ...
    const block = []
    const start = i
    while (i < officialLines.length && !isOrigLine(officialLines[i])) {
...
export function merge3(origSrc, patchedSrc, officialSrc) {
  ...
  // 1) 冲突检测：
  //    - orig 行补丁和官方**都删除** → 结果一致（都删），**不冲突**
  //    - 补丁删 + 官方保留（或反之）→ 跟随删除方，不冲突
  //    - 同一锚点（afterLine）双方插入**不同**内容 → 冲突（顺序/内容无法自动决定）
  const conflicts = []
  const patchIns = lineDiffInsertions(origLines, patchLines)
  const officialIns = lineDiffInsertions(origLines, officialLines)
  const patchAfterMap = new Map()
  for (const b of patchIns) {
    if (b.afterLine === null) continue
    if (!patchAfterMap.has(b.afterLine)) patchAfterMap.set(b.afterLine, [])
    patchAfterMap.get(b.afterLine).push(...b.block)
  }
  const officialAfterMap = new Map()
  for (const b of officialIns) {
    if (b.afterLine === null) continue
    if (!officialAfterMap.has(b.afterLine)) officialAfterMap.set(b.afterLine, [])
    officialAfterMap.get(b.afterLine).push(...b.block)
  }
  for (const anchor of patchAfterMap.keys()) {
    if (!officialAfterMap.has(anchor)) continue
    const patchText = patchAfterMap.get(anchor).join('\n')
    const officialText = officialAfterMap.get(anchor).join('\n')
    if (patchText !== officialText) {
      conflicts.push({
        line: Math.max(0, origLines.indexOf(anchor)) + 1,
        anchor: anchor.slice(0, 80),
        patch: patchText.slice(0, 120),
        official: officialText.slice(0, 120),
        reason: '补丁与官方都在同一行后插入不同内容（双方改动冲突）',
      })
    }
    // 相同内容 → 幂等，不冲突
  }
  if (conflicts.length > 0) {
    return { ok: false, conflicts, error: `三路合并冲突 ${conflicts.length} 处（补丁与官方在同一锚点插入不同内容），需人工适配` }
  }

  // 2) 生成合并结果：只有双方都保留的 orig 行输出（任一方删除则不输出），
  //    新增块按锚点插入；相同内容块去重（幂等）。
  const out = []
