...
async function adaptTemplateToOfficial(installDir, { dryRun = false } = {}) {
  ...
  if (!existsSync(origPath) || !existsSync(patchedPath) || !existsSync(officialPath)) {
    return { ok: false, error: '缺少 backup 模板或官方 app-boot 文件' }
  }
  const result = adaptPatchedTemplate(
    readFileSync(origPath, 'utf8'),
    readFileSync(patchedPath, 'utf8'),
    readFileSync(officialPath, 'utf8'),
  )
  if (!result.ok) return { ok: false, error: result.error, detail: result }

  // 语法校验：node --check 临时文件（必须通过，否则绝不写回）
  const tmp = join(installDir, '.dsh-fail-soft-adapt-check.js')
  writeFileSync(tmp, result.source)
  try {
    execFileSync(process.execPath, ['--check', tmp], { stdio: 'pipe' })
  } catch (e) {
    return { ok: false, error: `自适应生成的补丁语法校验失败：${String(e.stderr || e)}`, detail: result }
  } finally {
    try { unlinkSync(tmp) } catch { /* ignore */ }
  }

  if (dryRun) return { ok: true, message: result.note, detail: result }

  // 写回模板 + 应用：orig=官方新版（新锚点），patched=自适应结果，安装文件=patched
  writeFileSync(origPath, readFileSync(officialPath, 'utf8'))
  writeFileSync(patchedPath, result.source)
  writeFileSync(officialPath, result.source)
  return { ok: true, message: result.note, detail: result }
}
